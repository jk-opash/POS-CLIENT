...
function NavSectionTitle({ title }) {
  return (
    <p className="px-3 mt-5 mb-1 text-[10px] font-bold text-brand-muted uppercase tracking-widest">
      {title}
    </p>
  );
}
...
function SettingsNavLink({ label, icon, active, onClick, badge }) {
  return (
    ...
    >
      <div
        className={cn(
          ...
          active
            ...
            : "bg-brand-light text-brand-muted group-hover:bg-brand-border",
        )}
      >
        {icon}
      </div>
      ...
      {badge && (
        <span className="text-[10px] font-bold bg-brand-warningLight text-brand-warning px-1.5 py-0.5 rounded-md">
          {badge}
        </span>
      )}
    </button>
  ...
  return (
    <div className={cn("flex flex-col gap-1.5", full && "col-span-2")}>
      ...
      {badge ? (
        ...
        <div
          className={cn(
            "text-sm font-medium text-brand-dark bg-brand-bg px-3.5 py-2.5 rounded-xl border border-brand-border min-h-[42px] flex items-center",
            mono && "font-mono tracking-wider text-brand-dark",
          )}
        >
          {value || (
            <span className="text-brand-muted/70 font-normal italic">
              Not provided
            </span>
          )}
        </div>
...
function SectionHeader({ title, description, icon }) {
  return (
    <div className="flex items-start gap-3 pb-5 border-b border-brand-border mb-6">
      {icon && (
        <div className="p-2 bg-brand-light rounded-xl text-brand-primary shrink-0 mt-0.5">
          {icon}
        </div>
      )}
      <div>
    ...
    <div className="col-span-2 rounded-2xl border border-brand-primary/20 bg-gradient-to-br from-brand-light to-brand-bg p-5">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-xs font-bold text-brand-primary/70 uppercase tracking-widest">
              Current Plan
            </span>
          </div>
          <h4 className="text-xl font-extrabold text-brand-primary capitalize">
            {formatPlanName(plan.plan)}
          </h4>
          <p className="text-xs text-brand-primary/70 mt-0.5 capitalize">
            {plan.billing_cycle || "yearly"} billing &middot;{" "}
            {plan.amount > 0 ? `${plan.currency} ${plan.amount}` : "Free"}
          </p>
        </div>
        <div className="flex flex-col items-end gap-2">
          {isActive ? (
            <span className="text-[10px] font-bold text-brand-success bg-brand-successLight border border-brand-success/20 px-2.5 py-1 rounded-full uppercase tracking-wider">
              Active
            </span>
          ) : (
            <span className="text-[10px] font-bold text-brand-muted bg-brand-light border border-brand-border px-2.5 py-1 rounded-full uppercase tracking-wider">
              Inactive
            </span>
          )}
          {plan.cancel_at_period_end && (
            <span className="text-[10px] font-bold text-brand-warning bg-brand-warningLight border border-brand-warning/20 px-2.5 py-1 rounded-full uppercase">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-5">
        {[
          ...
          {
            label: "Period Start",
            value: formatDate(plan.current_period_start),
          },
          ...
          >
            <div className="text-[10px] text-brand-primary/70 font-semibold uppercase tracking-wider mb-1">
              {label}
            </div>
    ...
    <div className="rounded-xl border border-brand-border bg-brand-bg p-4 hover:border-brand-primary/30 hover:bg-brand-light/30 transition-all">
      ...
      <div className="space-y-1.5 text-xs text-brand-muted">
        <div className="flex items-center gap-1.5">
          <MapPin className="h-3 w-3 text-brand-muted/70 shrink-0" />
          {branch.address}, {branch.city}, {branch.state}
        </div>
        <div className="flex items-center gap-1.5">
          <Phone className="h-3 w-3 text-brand-muted/70 shrink-0" />
          {branch.contact}
        </div>
        <div className="flex items-center gap-1.5">
          <Mail className="h-3 w-3 text-brand-muted/70 shrink-0" />
          {branch.email}
        </div>
        <div className="flex items-center gap-1.5">
          <FileText className="h-3 w-3 text-brand-muted/70 shrink-0" />
          GST: {branch.tax_registration || "—"} ({branch.tax_percentage}%)
        </div>
        <div className="flex items-center gap-1.5">
          <Calendar className="h-3 w-3 text-brand-muted/70 shrink-0" />
          Opened: {formatDate(branch.opening_date)}
        </div>
        {branch.branch_type && (
          <div className="flex items-center gap-1.5">
            <Store className="h-3 w-3 text-brand-muted/70 shrink-0" />
            {branch.branch_type}
          </div>
        )}
        {branch.schedule && (
...
function TeamMemberRow({ member }) {
  ...
  const roleColor =
    { Manager: "purple", Cashier: "blue", Waiter: "green" }[roleName] ||
    "default";
  return (
    <div className="flex items-center gap-3 py-3.5 px-4 border-b border-brand-border last:border-0 hover:bg-brand-bg transition-colors">
      ...
      <div className="flex items-center gap-2 shrink-0">
        <span className="text-[10px] text-brand-muted font-mono hidden sm:block">
          BR-{branchLabel}
        </span>
        ...
        <span
          className={cn(
            ...
            member.status === "Active"
              ? "bg-brand-successLight text-brand-success"
          ...
          <div className="space-y-8">
            <div>
              ...
              <div className="grid grid-cols-2 gap-5">
                ...
                <Field
                  label="Role"
                  badge={
                    <span className="text-[11px] font-bold text-brand-purple bg-brand-purple/10 border border-brand-purple/20 px-2.5 py-1 rounded-full uppercase">
                      {user?.role || "admin"}
                    </span>
                  }
                />
                ...
                <Field
                  ...
                  badge={
                    user?.is_active ? (
                      <span className="text-[11px] font-bold text-brand-success bg-brand-successLight border border-brand-success/20 px-2.5 py-1 rounded-full uppercase">
                        Active
                      </span>
                    ) : (
                      <span className="text-[11px] font-bold text-brand-danger bg-brand-dangerLight border border-brand-danger/20 px-2.5 py-1 rounded-full uppercase">
                        Suspended
                      </span>
                    )
                  }
            ...
            <div>
              <SectionHeader
                ...
                description="Your current active plan and usage limits"
                icon={<CreditCard className="h-4 w-4" />}
              />
              <div className="grid grid-cols-2 gap-5">
                <PlanCard plan={subscription} />
                <Field
                  label="Extra Branches Purchased"
                  value={String(business?.extra_branches ?? 0)}
                  icon={<GitBranch className="h-3 w-3" />}
                />
                <Field
                  label="Extra Team Members Purchased"
                  value={String(business?.extra_team_members ?? 0)}
                  icon={<Users className="h-3 w-3" />}
                />
                <Field
                  label="Total Branch Capacity"
                  value={`${(subscription?.max_branches ?? 0) + (business?.extra_branches ?? 0)} branches`}
                />
                <Field
                  label="Total Team Member Capacity"
                  value={`${(subscription?.max_team_members ?? 0) + (business?.extra_team_members ?? 0)} members`}
                />
              </div>
            </div>
            <InfoBanner>
              To upgrade your plan or purchase additional branches and team
              members, please contact your account administrator.
            </InfoBanner>
          </div>
        );
          ...
          <div className="space-y-8">
            <div>
              ...
              <div className="grid grid-cols-2 gap-5">
                ...
                <Field
                  label="Business Type"
                  value={business?.business_type?.replace(/\b\w/g, (c) =>
                    c.toUpperCase(),
                  )}
                />
                    ...
                    <span
                      className={cn(
                        ...
                        business?.status === "active"
                          ? "text-brand-success bg-brand-successLight border-brand-success/20"
                          : "text-brand-muted bg-brand-light border-brand-border",
                      )}
                    >
                      {business?.status || "—"}
                    </span>
            ...
            <div>
              <SectionHeader
                ...
                description="Legal address of your business"
            ...
            <div>
              <SectionHeader
                ...
                description="Personal information of the business owner"
                icon={<User className="h-4 w-4" />}
              />
              <div className="grid grid-cols-2 gap-5">
                <Field
                  label="Date of Birth"
                  value={formatDate(business?.date_of_birth)}
                  icon={<Calendar className="h-3 w-3" />}
                />
                <Field
                  label="Gender"
                  value={business?.gender?.replace(/\b\w/g, (c) =>
                    c.toUpperCase(),
                  )}
                />
              </div>
          ...
          <div className="space-y-8">
            <div>
              <SectionHeader
                ...
                description="Your business tax registrations and identifiers"
                ...
                <Field
                  ...
                  badge={
                    <span
                      className={cn(
                        "text-[11px] font-bold px-2.5 py-1 rounded-full uppercase border",
                        business?.identity_verification === "verified"
                          ? "text-brand-success bg-brand-successLight border-brand-success/20"
                          : business?.identity_verification === "pending"
                            ? "text-brand-warning bg-brand-warningLight border-brand-warning/20"
                            : "text-brand-danger bg-brand-dangerLight border-brand-danger/20",
                      )}
                    >
                      {business?.identity_verification || "Pending"}
                    </span>
            ...
            <div>
              <SectionHeader
                title="Branch-wise Tax Configuration"
                description="GST rate and jurisdiction for each branch"
                  ...
                  <thead className="bg-brand-bg border-b border-brand-border">
                    <tr>
                      {[
                        ...
                        <th
                          key={h}
                          className="py-3 px-4 text-xs font-bold text-brand-muted uppercase tracking-wider"
                        >
                          {h}
                        </th>
                      ))}
                  ...
                  <tbody>
                    {branches.length === 0 ? (
                      ...
                      branches.map((b) => (
                        <tr
                          key={b.id}
                          className="border-b border-brand-border last:border-0 hover:bg-brand-bg transition-colors"
                        >
                          <td className="py-3 px-4 font-semibold text-brand-dark">
                            {b.name}
                          </td>
                          <td className="py-3 px-4 font-mono text-brand-muted text-xs">
                            {b.code}
                          </td>
                          <td className="py-3 px-4 text-brand-dark">
                            {b.tax_jurisdiction || "—"}
                          </td>
                          <td className="py-3 px-4 font-mono text-brand-dark text-xs">
                            {b.tax_registration || "—"}
                          </td>
            ...
            <InfoBanner>
              Tax configuration per branch is managed by your account
              administrator. Contact support to update jurisdiction or
        ...
        return (
          <div className="space-y-6">
            <SectionHeader
              ...
              description={`${branches.length} branch${branches.length !== 1 ? "es" : ""} under your business`}
      ...
      case "team":
        return (
          <div className="space-y-6">
            <SectionHeader
              title="Team Members"
              description={`${teamMembers.length} member${teamMembers.length !== 1 ? "s" : ""} across all branches`}
              ...
              <div className="w-full md:w-60 shrink-0 flex flex-col gap-5">
                ...
                <nav className="flex flex-col gap-0.5">
                  ...
                  <SettingsNavLink
                    ...
                    badge={branches.length > 0 ? String(branches.length) : null}
                  />
                  <SettingsNavLink
                    label="Team Members"
                    ...
                    onClick={() => setTab("team")}
                    badge={
                      teamMembers.length > 0 ? String(teamMembers.length) : null
                    }
                  />
                </nav>
              </div>
