      ...
      <div className="modal" onClick={(e) => e.stopPropagation()}>
        ...
        >
          ...
          <div>
            ...
            <div style={{ fontSize: 13, color: "var(--color-text-primary)" }}>
              {PAYMENT_ICONS[invoice.paymentMethod]} {invoice.paymentMethod}
            </div>
          </div>
        </div>
...
export default function InvoicesPage() {
  ...
  const todayGST = invoices
    .filter(
      (inv) => new Date(inv.createdAt).toDateString() === now.toDateString(),
    )
    .reduce((s, inv) => s + ((inv.cgst || 0) + (inv.sgst || 0)), 0);
        ...
        <main style={{ flex: 1, overflowY: "auto", padding: "20px 24px" }}>
          ...
          <div style={{ display: "flex", gap: 10, marginBottom: 20 }}>
            ...
            <div className="tab-bar">
              <button
                className={`tab-item ${dateFilter === "all" ? "active" : ""}`}
                onClick={() => setDateFilter("all")}
              >
                All
              </button>
              <button
                className={`tab-item ${dateFilter === "today" ? "active" : ""}`}
                ...
                {filtered.map((inv) => {
                  ...
                  return (
                    <tr key={inv.id}>
                      ...
                      <td>
                        <span
                          className={`badge badge-${inv.type === "dine-in" ? "sky" : inv.type === "takeaway" ? "amber" : "violet"}`}
                        >
                          {inv.type}
                        </span>
                      </td>
                      ...
                      <td>
                        {PAYMENT_ICONS[inv.paymentMethod]} {inv.paymentMethod}
                      </td>
                      <td
                        style={{
                          color: "var(--color-text-muted)",
