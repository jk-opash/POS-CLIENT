...
export default function DayEndReport() {
  ...
  return (
    <>
      ...
      {/* Z-REPORT PRINT VIEW - 80mm Thermal Receipt Format */}
      <div style={{ display: "none" }}>
        <div
          ref={componentRef}
          className="font-mono text-black bg-white w-[80mm] mx-auto p-4 text-xs leading-relaxed"
        >
          <div className="text-center mb-6">
            <h2 className="font-bold text-xl mb-1 uppercase">
              {branches?.find((b) => b.id === branchFilter)?.name || "Z-REPORT"}
            </h2>
            <p className="uppercase font-semibold">End of Day Report</p>
            <p>{new Date().toLocaleString("en-IN")}</p>
            <p>Printed by: {user?.name || "Admin"}</p>
          </div>

          <div className="border-b-2 border-black border-dashed pb-1 mb-2">
            <h3 className="font-bold text-center uppercase tracking-widest text-sm">
              Sales Summary
            </h3>
          </div>
          ...
          <div className="flex justify-between">
            <span>Taxes</span>
            <span>{fmt(taxCollected)}</span>
          </div>

          <div className="border-b-2 border-black border-dashed pb-1 mt-4 mb-2">
            <h3 className="font-bold text-center uppercase tracking-widest text-sm">
              Tendered
            </h3>
          </div>
          ...
          <div className="flex justify-between">
            ...
            <span>+{fmt(cashCol)}</span>
          </div>
          {pettyCash.map((pc, i) => (
            <div key={i} className="flex justify-between">
              <span>Pay out: {pc.label}</span>
              <span>-{fmt(pc.val)}</span>
            </div>
          ))}
          <div className="flex justify-between font-bold mt-2 border-t-2 border-black pt-1 text-sm">
          ...
          <div className="mt-8 text-center border-t-2 border-black border-dashed pt-4">
            <p className="uppercase font-bold">*** End of Report ***</p>
          </div>
        </div>
      </div>
    </>
