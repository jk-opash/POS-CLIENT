...
import { X, Save, Building2, User, Landmark } from "lucide-react";
...
export default function SupplierModal({ visible, supplier, onClose, onSave }) {
  ...
  const validateForm = () => {
    ...
    if (
      ...
      !/^\d{10}$/.test(formData.contact_mobile.replace(/\D/g, ""))
    ) {
    ...
    if (
      ...
      !/^[\w-\.]+@([\w-]+\.)+[\w-]{2,4}$/.test(formData.contact_email)
    ) {
    ...
    if (
      ...
      !/^https?:\/\/.+/.test(formData.contact_website)
    ) {
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setFormData({ ...formData, tax_gst: e.target.value })
                      }
