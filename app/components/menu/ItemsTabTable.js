...
export default function ItemsTabTable({
  ...
  const getFoodBadgeVariant = (foodType, isVeg) => {
    ...
    if (type.includes("beverage") || type.includes("drink"))
      return { variant: "purple", label: "Beverage" };
                      ...
                      <div className="flex items-center gap-1.5 flex-wrap">
                        {item.variants?.length > 0 && (
                          <div className="group relative flex items-center cursor-default">
                            ...
                            <div className="absolute z-[70] bottom-full left-1/2 -translate-x-1/2 mb-3 z-50 pointer-events-none opacity-0 invisible translate-y-2 group-hover:opacity-100 group-hover:visible group-hover:translate-y-0 transition-all duration-200 ease-out">
                              <div className="bg-brand-info text-white text-[11px] font-medium p-3 rounded-xl shadow-lg border border-brand-info w-max max-w-[240px] whitespace-pre-wrap text-left leading-relaxed relative">
                                {item.variants
                                  ...
                                  .join("\n")}
                                <div className="absolute -bottom-1.5 left-1/2 -translate-x-1/2 w-3 h-3 bg-brand-info text-brand-dark border border-brand-info/50 rotate-45"></div>
                              </div>
                            </div>
                          </div>
                        )}
