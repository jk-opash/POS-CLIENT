  ...
  return (
    <div className="min-h-screen bg-brand-bg flex relative">
      ...
      <div className="hidden lg:flex lg:w-1/2 flex-col justify-between p-12 relative overflow-hidden">
        {/* Background gradient */}
        <div className="absolute inset-0 bg-gradient-to-br from-brand-dark/20 via-brand-bg to-brand-primaryLight/30" />
        ...
        <div className="relative space-y-6">
          <h2 className="text-4xl font-bold text-brand-dark leading-tight">
            ...
            <br />
            <span className="bg-gradient-to-r from-brand-dark to-brand-primaryDark bg-clip-text text-transparent">
              Point of Sale
            </span>
          </h2>
          <p className="text-brand-muted leading-relaxed max-w-sm">
            Manage your restaurant operations, orders, and staff from one
            powerful platform.
          </p>
        ...
        <div className="relative">
          <p className="text-xs text-brand-placeholder">
            © 2026 POS Client. All rights reserved.
          </p>
        </div>
      </div>
      ...
      <div className="flex flex-1 items-center justify-center p-8 z-10 relative">
        <Card className="w-full max-w-sm" padding="lg">
          ...
          {/* Security badge */}
          <div className="mb-6 flex items-center gap-2 rounded-lg border border-brand-success/20 bg-brand-successLight/30 px-3 py-2">
            <ShieldCheck className="h-4 w-4 text-brand-success" />
            <span className="text-xs text-brand-success">
              Secure access — Owners only
            </span>
          </div>
          ...
          <form onSubmit={handleLogin} className="space-y-4" id="login-form">
            ...
            {error && (
              <p className="text-sm text-brand-danger bg-brand-dangerLight/30 border border-brand-danger/20 rounded-lg px-3 py-2">
                {error}
              </p>
            )}
